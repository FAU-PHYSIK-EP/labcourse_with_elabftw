...
def get_experiments(searchstr: str='', tags=[]):
    global __APICLIENT__
    apiexperiments = elabapi_python.ExperimentsApi(__APICLIENT__)
    
    return apiexperiments.read_experiments(q=searchstr, tags=tags, limit=9999)
...
def __get_upload_id_by_long_name(expid: int, long_name: str):
    global __APICLIENT__
    apiuploads = elabapi_python.UploadsApi(__APICLIENT__)

    # fetch metadata of all uploads and search for long name
    uploads = apiuploads.read_uploads('experiments', expid)
    uploadid = None
...
def save_file_longname(filename: str, savestr: str, expid: int):
    global __APICLIENT__
    apiuploads = elabapi_python.UploadsApi(__APICLIENT__)
    ...
    if uploadid is None:
        ...
        # fetch file data
        pic = apiuploads.read_upload('experiments', expid, uploadid, format='binary', _preload_content=False).data
        with open(savestr, 'wb') as f:
            f.write(pic)
            #print('Image successfully downloaded', savestr)
            f.close()

def get_uploads(expid: int):
    global __APICLIENT__
    apiuploads = elabapi_python.UploadsApi(__APICLIENT__)
